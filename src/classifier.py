...
from sklearn.metrics import confusion_matrix, roc_curve, recall_score, precision_score, accuracy_score, f1_score
...
class RandForest():
    ...
    def chart_gini_import(self, features=-1, vocab=None):
        ...
        if features == -1:
            ...
            feature_scores = feature_scores.iloc[:features].sort_values()
            ax = feature_scores.plot(kind='barh', figsize=(10,4))
            ax.set_title('Gini Importance')
            ax.set_xlabel('Avg. Contribution to Info Gain');
