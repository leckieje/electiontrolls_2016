...
Given this evidence, I believed there was a reasonable chance I could begin to classify the Tweets by topic. I count vectorized my corpus, ran a Latent Dirichlet Allocation model, and then re-introduced the labels to view the distribution of topics by class. But the model showed no discernible difference in the distribution of topics among each class. The charts below show the results with five topics, but the models were similar no matter the number of topics used. Upon reflection, this finding shouldn't have been such a shock. From what revelations of congressional investigations into the operation, the troll's goal was not to introduce new topics but to exploit topics that already existed. This meant that even if trolls favored words like `police`, `sports`, and `black`, racial injustice was not a topic unique to them--even if they were more likely to poke at the topic--and distinguishing the classes based on topics was unlikely to succeed. 
...
While these words and the differences in their frequency of use proved important to classifying the Tweets, the presence of the word did not guarantee a correct classification. Below are two Tweets about the police that the model misclassified. Both Tweets could be classified as right-wing, pro-police, and at best diminish, if not glorify, violence. It's easy to see why the model would have trouble, although it is interesting to note the Tweets come from different classes and for each class the model classified incorrectly. See if you can correctly identify them. 
